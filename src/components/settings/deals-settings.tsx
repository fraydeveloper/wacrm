...
export function DealsSettings() {
  ...
  async function handleSave() {
    ...
    const { error } = await supabase
      ...
      .eq("id", accountId);
    if (error) {
      toast.error("No se pudo guardar la moneda predeterminada");
      setSaving(false);
      return;
    }
  ...
  return (
    <section className="max-w-2xl animate-in fade-in-50 duration-200">
      <SettingsPanelHead
        ...
        description="La moneda usada para los nuevos negocios y para los totales del pipeline y del panel."
      />
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-muted-foreground">
            Los nuevos negocios usan esta moneda de forma predeterminada, y los totales
            del pipeline y del panel se muestran en ella. Los negocios existentes conservan la
            moneda con la que se guardaron.
          </CardDescription>
        </CardHeader>

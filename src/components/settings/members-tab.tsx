...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  { value: 'admin', label: 'Administrador', hint: 'Gestiona miembros y todo lo demás' },
  { value: 'agent', label: 'Agente', hint: 'Usa las funciones; sin configuración' },
  { value: 'viewer', label: 'Visualizador', hint: 'Solo lectura en toda la app' },
];
...
export function MembersTab() {
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      toast.success(
        `Se actualizó a ${member.full_name || 'miembro'} a ${ROLE_META[nextRole].label}`,
      );
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      toast.success(`Se eliminó a ${removingMember.full_name || 'miembro'}`);
      setMembers((prev) =>
  ...
  return (
    <section className="animate-in fade-in-50 space-y-6 duration-200">
      <SettingsPanelHead
        ...
        description="Personas con acceso a esta cuenta. Los roles controlan qué puede hacer cada compañero."
        action={
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              La URL de invitación en texto plano solo se muestra una vez al
              crearla, por seguridad — para volver a compartir, revoca la
              invitación de abajo y crea una nueva.
            </p>
          ) : null}
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Haz clic en <span className="text-muted-foreground">Invitar miembro</span>{' '}
                  arriba para generar un enlace compartible.
                </p>
              </CardContent>
            </Card>
          ) : (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-foreground">
                            {inv.label || 'Invitación sin título'}
                          </span>
                          <span
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              de la cuenta? Se cerrará su sesión en esta cuenta
              y se le asignará una cuenta personal nueva en su próximo inicio de sesión. Su
              acceso no se elimina.
            </DialogDescription>
          </DialogHeader>

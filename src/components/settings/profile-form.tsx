...
export function ProfileForm() {
    ...
    try {
      ...
      if (trimmedEmail.toLowerCase() !== profile.email.toLowerCase()) {
        ...
        if (emailError) {
          ...
          toast.error(`Error al cambiar el correo: ${emailError.message}`);
          setSaving(false);
  ...
  return (
    <section className="max-w-2xl animate-in fade-in-50 duration-200">
      <SettingsPanelHead
        title="Tu perfil"
        description="Cómo apareces en toda la app. Tu avatar y nombre se muestran en el encabezado, la barra lateral y donde sea que tus compañeros te vean."
      />
          ...
          <div className="space-y-2">
            ...
            {emailChangePending && (
              <p className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
                ...
                <span>
                  Revisa la bandeja de entrada de <strong>{profile?.email}</strong> y{' '}

...
export function ImportModal({
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (tagsAssigned > 0) {
        toast.success(
          `${tagsAssigned} asignaci${tagsAssigned !== 1 ? 'ones' : 'ón'} de etiqueta aplicada${tagsAssigned !== 1 ? 's' : ''}`
        );
      }
      ...
      if (failed > 0) {
        toast.error(
          `Error al importar ${failed} contacto${failed !== 1 ? 's' : ''}`
        );
      }
    } catch (err: unknown) {
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              Sube un CSV con una columna obligatoria{' '}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                {result.failed > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-red-400">
                    ...
                    {result.failed} fallido{result.failed !== 1 ? 's' : ''}
                  </div>
                )}
              </div>
            </div>

      ...
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            Define campos adicionales para tus contactos (ej. código postal,
            fuente del lead). Aparecen en cada contacto y en la acción de
            automatización “Update Contact Field”.
          </DialogDescription>
        </DialogHeader>
        <CustomFieldsPanel />
      </DialogContent>
...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (error) {
      toast.error('No se pudo crear el campo. Es posible que no tengas permiso.');
      return;
    }
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        `¿Eliminar "${field.field_name}"? Esto también eliminará su valor guardado en todos los contactos. Esta acción no se puede deshacer.`
      )
    ) {
      return;
    }
    ...
    toast.success(`"${field.field_name}" eliminado.`);
    await fetchFields();
  }
